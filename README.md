...
- Filtro BR que confirma o país do servidor antes do teleporte
...
- O filtro Servidor BR consulta o endpoint de junção do Roblox e uma API de geolocalização para confirmar o país. Se o Roblox responder HTTP 401 nessa consulta, o script cancela a busca sem teleportar, evitando entrar em um servidor de outro país.
...
- O executor precisa suportar requisições HTTP GET e POST. A busca de amigos usa POST em `presence.roblox.com`; o filtro BR usa o POST em `gamejoin.roblox.com`. Esse endpoint pode exigir autenticação e responder HTTP 401; nesse caso o script cancela a busca e mostra uma mensagem explicando que nenhum teleporte foi feito.
